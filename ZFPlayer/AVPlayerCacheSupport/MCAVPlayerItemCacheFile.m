...
+ (void)removeAllCachedFile
{
    NSFileManager *fm = [NSFileManager defaultManager];
    NSArray *files = [fm contentsOfDirectoryAtPath:MCCacheTemporaryDirectory() error:NULL];
    for (NSString *item in files) {
        NSError *error;
        [fm removeItemAtPath:[MCCacheTemporaryDirectory() stringByAppendingPathComponent:item] error:&error];
...
+ (long)cachedFileSize
{
    NSString *path = MCCacheTemporaryDirectory();
    return [[FCFileManager sizeOfDirectoryAtPath:path error:NULL] longValue];
}
